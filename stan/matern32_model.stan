functions {
  ...
  vector gp_pred_rng(array[] real x_star,
    ...
    vector[N_star] f_star;
    {
      ...
      K = eta * gp_matern32_cov(x, 1.0, ell);
      for (n in 1:N)
      ...
      k_x_xstar = eta * gp_matern32_cov(x, x_star, 1.0, ell);
      fstar_mu = k_x_xstar' * alpha;

      v = mdivide_left_tri_low(L, k_x_xstar);
      fstar_cov = eta * gp_matern32_cov(x_star, 1.0, ell) - v' * v;
...
model {
  matrix[N, N] K = eta * gp_matern32_cov(x, 1.0, ell);
