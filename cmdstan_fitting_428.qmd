...
```{r}
fit$print(variables = c("eta", "ell", "sigma"))
```

```{r}
fit_draws <- as_draws_rvars(fit$draws())
fit_draws
```

```{r}
mcmc_hist(fit$draws(c("ell", "eta", "sigma")))
```

```{r}
post_pred_draws <- as_draws_rvars(fit$draws("f_star"))

ggplot() +
  geom_line(aes(x = x_star, y = median(post_pred_draws$f_star)), colour = "red") +
