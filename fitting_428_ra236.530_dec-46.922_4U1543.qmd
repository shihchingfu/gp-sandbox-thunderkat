...
x_star <- seq(from = min(x), to = max(x), length.out = 200)
...
```{r}
err_fit$print(variables = c("eta", "ell", "sigma[1]"), digits = 5)
```
...
```{r}
m32_fit$diagnostic_summary()
```

## MCMC Results

```{r}
m32_fit$print(variables = c("eta", "ell", "sigma[1]"), digits = 5)
```
...
```{r}
plot_postpred(m32_fit, x, y, y_err, x_star)
```
...
$$\ell_\mathrm{SE} \sim \mathrm{InvGamma}(3, 8 \times \lceil\textrm{min x gap}\rceil)$$

$$\ell_\mathrm{M} \sim \mathrm{InvGamma}(3, 8 \times \lceil\textrm{min x gap}\rceil)$$
...
```{r}
plot_postpred(SE_M32_fit, x, y, y_err, x_star)
```
...
```{r}
SE_M32_2_fit$diagnostic_summary()
```

## MCMC Results

```{r}
SE_M32_2_fit$print(variables = c("eta_SE", "eta_M", "ell_SE", "ell_M", "sigma[1]"), digits = 4)
```
...
```{r}
SExM32_fit$diagnostic_summary()
```

## MCMC Results

```{r}
SExM32_fit$print(variables = c("eta", "ell_SE", "ell_M", "sigma[1]", "f_star[1]"), digits = 5)
```
...
```{r fig.height=9}
mcmc_dens(SExM32_fit$draws(), regex_pars = "sigma", facet_args = list(nrow = 6))
```

## Posterior Predictive Samples

```{r}
plot_postpred(SExM32_fit, x, y, y_err, x_star)
```
...
$$\ell_\mathrm{P} \sim \mathrm{InvGamma}(3, 8 \times \lceil\textrm{min x gap}\rceil)$$
...
```{r}
SE_P_fit$print(variables = c("eta", "ell_SE", "ell_P", "T"), digits = 4)
```

## MCMC Plots

```{r}
mcmc_plots(SE_P_fit, c("eta", "ell_SE", "ell_P", "T"))
```

```{r fig.height=9}
mcmc_dens(SE_P_fit$draws(), regex_pars = "sigma", facet_args = list(ncol = 5))
```
...
```{r fig.height=9}
mcmc_dens(SE_M32_P_fit$draws(), regex_pars = "sigma", facet_args = list(ncol = 5))
```
