...
plot_postpred <- function(fit, x, y, y_err, x_star) {
...
plot_PSD <- function(fit, nsamples = 200) {
  draws_subset <- 
...
- The light curve has $N =$ `r NROW(data_df)` observations over a range of `r diff(range(x))` days.
- The mean flux density is $\bar{y} =$ `r mean(y)` Jy.
- The mean standard error is `r mean(y_err)` Jy.
...
```{r}
plot_postpred(fixed_mean_fit, x, y, y_err, x_star) + 
  geom_hline(aes(yintercept = 0.2), colour = "orange") 
```

```{r}
#| message: false
plot_PSD(fixed_mean_fit)
```
...
```{r}
plot_postpred(fixed_mean_fit, x, y, y_err, x_star) + 
  geom_hline(aes(yintercept = 0.195), colour = "orange") 
```
...
```{r}
plot_postpred(fixed_mean_fit, x, y, y_err, x_star) + 
  geom_hline(aes(yintercept = 0.18), colour = "orange") 
```
